...
# model.eval()
# # 选择模型中的目标层
# target_layer = model.conv1  # Ensure this is a convolutional layer
#
# # 创建Grad-CAM对象
...
#
# input_image = torch.stack(X_test, dim=1).unsqueeze(1).to(device)
...
#
# # 检查生成的CAM
# if np.isnan(cam).all() or np.max(cam) == np.min(cam):
#     print("CAM生成错误，请检查目标层或梯度计算")
# else:
#     # 可视化所有导联
...
#         plt.plot(lead_data, label=f'Lead {i + 1}')
#
#         # 生成与ECG信号相同长度的CAM
...
# 加载模型
model = CapsuleNetwork().to(device)
model.load_state_dict(torch.load('capsule_network_629.pth'))
model.eval()

# 选择模型中的目标层
target_layer = model.conv1  # 确保这是一个卷积层
...
# 提取每个导联的Grad-CAM
cam_all_leads = []
for i in range(12):  # 对每个导联分别计算Grad-CAM
    lead_image = sample_image[:, :, i, :].unsqueeze(2)  # 提取当前导联
    cam = grad_cam.generate_cam(lead_image, sample_target.item())
    cam_all_leads.append(cam)
...
for i in range(num_leads):
    plt.subplot(num_leads, 1, i + 1)
    lead_data = X_test[i][sample_index].cpu().numpy()
    plt.plot(lead_data, label=f'Lead {i + 1}', color='#7b92c7')

    # 使用每个导联的CAM
    cam_resized = cv2.resize(cam_all_leads[i], (lead_data.shape[0], 1)).squeeze()
    cam_resized = cam_resized
    plt.imshow(cam_resized[np.newaxis, :], cmap='Purples', alpha=0.5, aspect='auto',
               extent=(0, lead_data.shape[0], np.min(lead_data), np.max(lead_data)))

    plt.gca().set_facecolor('#c2c1e0')  # 背景色为紫色 #830783
    plt.legend(loc='upper right')
